window.addEventListener("load", () => {
  ...
  gameButton.addEventListener("click", gameButtonClick.bind(gameButton, gameSecondTime));
  ...
  function addItem(className, count, imagePath) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      item.addEventListener("click", itemClick.bind(item, className));
  ...
  function gameButtonClick(second) {
    const _this = this;
    if (_this.querySelector("i").getAttribute("class").indexOf("play") > -1) {
      startTimer(second);
      //_this.setAttribute("class", "fas fa-stop");
    } else {
      //_this.setAttribute("class", "fas fa-play");
    }
  }
});
